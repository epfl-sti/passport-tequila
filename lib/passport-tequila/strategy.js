                ...
                if (error) {
                    ...
                    req.login(teqResult2User(results), function(error) {
                        if (error) {
                            ...
                            customVerify(verify, req, res, next, function (req, res) {
...
function customVerify(verifyFunc, req, res, fail, success) {
    if (util.isFunction(verifyFunc)) {
        verifyFunc(req, res, function(err, req, res) {
            if (error) {
                fail(error);
            } else {
                success( req, res);
